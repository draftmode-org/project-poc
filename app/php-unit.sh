...
EXECUTE="${OPTIONS[*]}"
echo "[$(date)]<I> start execute command: docker exec -it ${CONTAINER} ${BASH_CMD} ./${VENDOR_FOLDER}/bin/phpunit ${EXECUTE}"
start=(date +%s)
if [[ -z ${PREVIEW} ]]; then
  docker exec -it "${CONTAINER}" "${BASH_CMD}" "./${VENDOR_FOLDER}/bin/phpunit ${EXECUTE}"
fi
end=(date +%s)
runtime=$((end-start))
